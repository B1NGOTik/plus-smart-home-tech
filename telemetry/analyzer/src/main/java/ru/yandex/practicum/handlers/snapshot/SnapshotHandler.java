...
public class SnapshotHandler {
    ...
    private Boolean checkCondition(Condition condition, Map<String, SensorStateAvro> sensorStateMap) {
        ...
        switch (condition.getType()) {
            case LUMINOSITY -> {
                LightSensorEventAvro lightSensor = (LightSensorEventAvro) sensorState.getData();
                return handleOperation(condition, lightSensor.getLuminosity());
            }
            case TEMPERATURE -> {
                ClimateSensorEventAvro temperatureSensor = (ClimateSensorEventAvro) sensorState.getData();
                return handleOperation(condition, temperatureSensor.getTemperatureC());
            }
            case MOTION -> {
                MotionSensorEventAvro motionSensor = (MotionSensorEventAvro) sensorState.getData();
                return handleOperation(condition, motionSensor.getMotion() ? 1 : 0);
            }
            case SWITCH -> {
                SwitchSensorEventAvro switchSensor = (SwitchSensorEventAvro) sensorState.getData();
                return handleOperation(condition, switchSensor.getState() ? 1 : 0);
            }
            case CO2LEVEL -> {
                ClimateSensorEventAvro climateSensor = (ClimateSensorEventAvro) sensorState.getData();
